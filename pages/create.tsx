...
export default function Create() {
  ...
  const [keyboardKind, setKeyboardKind] = useState<number>(0);
  const [isPBT, setIsPBT] = useState<boolean>(false);
  ...
  const submitCreate = async (e: React.FormEvent<HTMLInputElement>) => {
    e.preventDefault();
      ...
      <form className="mt-8 flex flex-col gap-y-6">
        <div>
          ...
          <select
            ...
            value={keyboardKind}
            onChange={(e:React.ChangeEvent<HTMLSelectElement>) => { setKeyboardKind(parseInt(e.target.value)) }}
          >
